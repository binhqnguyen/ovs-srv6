...
#define SR
#ifdef SR
#define MAX_IPV6_SEGMENTS 4	//Must be lest than 5 because the total struct flow must be less than 260B in size.
#endif
...
//BN
#ifdef SR
    struct in6_addr ipv6_segments[MAX_IPV6_SEGMENTS];   /* IPv6 segments address. */
...
#else
/* Remember to update FLOW_WC_SEQ when changing 'struct flow'. */
BUILD_ASSERT_DECL(sizeof(struct flow) == (sizeof(struct flow_tnl) + 160 + sizeof(struct in6_addr)*MAX_IPV6_SEGMENTS + sizeof(uint32_t) + 4) && FLOW_WC_SEQ == 20);
#endif
