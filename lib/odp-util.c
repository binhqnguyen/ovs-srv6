                       ...
                       struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    commit_set_action(odp_actions, OVS_KEY_ATTR_IPV6,
                      &ipv6_key, sizeof(ipv6_key));
    //BN
    //inet_ntop(AF_INET6, &ipv6_key.ipv6_dst, str, INET6_ADDRSTRLEN);
    //VLOG_WARN("commit_set_ipv6_action\n");
}
...
static void
commit_set_ipv6_segment(const struct flow *flow, struct flow *base,
                       struct ofpbuf *odp_actions, const uint32_t index, struct flow_wildcards *wc)
{
    struct ovs_key_ipv6 ipv6_key;
    ...
    memcpy(&ipv6_key.ipv6_src, &base->ipv6_src, sizeof(ipv6_key.ipv6_src));
    base->ipv6_dst = flow->ipv6_dst;
    //ipv6_key.ipv6_dst store the ipv6 segment.
    memcpy(&ipv6_key.ipv6_dst, &flow->ipv6_segments[index], sizeof(ipv6_key.ipv6_dst));
    ...
    commit_set_action(odp_actions, OVS_KEY_ATTR_IPV6,
                      &ipv6_key, sizeof(ipv6_key));
    VLOG_WARN("commit_set_ipv6_segment, index = %d\n", index);
}
                     ...
                     struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
	...
	VLOG_ERR("ERR: Packets are not IPv6!\n");
    }
}
                   ...
                   struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    //commit_set_nw_action(flow, base, odp_actions, wc);
    commit_set_ipv6_segments_action(flow, base, odp_actions, wc);
