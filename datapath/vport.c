...
static const struct vport_ops *vport_ops_list[] = {
	...
	&ovs_vxlan_vport_ops,
	&ovs_lisp_vport_ops,
	&ovs_srv6_vport_ops, 		// add srv6 vport
};
...
#ifdef _SR_
/**
 *	ovs_vport_send_segments - send a packet on a device with ipv6 segments.
 *
 * @vport: vport on which to send the packet
 * @skb: skb to send
 * @segments: ipv6 segments used to encapsulate the ipv6 packets
 *
 * Sends the given packet and returns the length of data sent.  Either ovs
 * lock or rcu_read_lock must be held.
 */
int ovs_vport_send_segments(struct vport *vport, struct sk_buff *skb, struct ipv6_segments *segments)
{
	int sent = vport->ops->send_with_segments(vport, skb, segments);
