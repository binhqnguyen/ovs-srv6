...
void ovs_dp_process_received_packet(struct vport *p, struct sk_buff *skb)
{
	...
	pr_warn("BN ovs_dp_process_received_packet, port = %d, in_port = %d\n", p->port_no, key.phy.in_port);
	if (unlikely(error)) {
		kfree_skb(skb);
		return;
	}
	...
	if (unlikely(!flow)) {
		//BN
		pr_warn("BN ovs_dp_process_received_packet: NOT found, to user, port = %d\n", p->port_no);
		...
		//BN
		pr_warn("BN ovs_dp_process_received_packet: FOUND, port = %d\n", p->port_no);
	}
				  ...
				  const struct dp_upcall_info *upcall_info)
{
	...
	//BN
	pr_warn("BN queue_userspace_packet\n");
...
static int ovs_packet_cmd_execute(struct sk_buff *skb, struct genl_info *info)
{
	...
	//BN
	pr_warn("\n\nNETLINK MSG: ovs_packet_cmd_execute\n");
	err = -EINVAL;
...
static int ovs_flow_cmd_new_or_set(struct sk_buff *skb, struct genl_info *info)
{
	...
	int error;
        ...
        //BN
        pr_warn("\n\n NETLINK MSG: ovs_flow_cmd_new_or_set\n");
	/* Extract key. */
	error = -EINVAL;
	...
	if (a[OVS_FLOW_ATTR_ACTIONS]) {
		...
		error = validate_and_copy_actions(a[OVS_FLOW_ATTR_ACTIONS],
						  &masked_key, 0, &acts);
		//BN
		pr_warn("ovs_flow_cmd_new_or_set: actions len = %d\n", acts->actions_len);
		if (error) {
	...
	if (!flow) {
		//BN
		pr_warn("BN ovs_flow_cmd_new_or_set: !FLOW\n");
